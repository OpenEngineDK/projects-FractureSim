...
calculateDrivingForces_k(Point *points, float *masses, float4 *externalForces, unsigned int numPoints)
{
	...
	//	externalForces[me_idx] = make_float4(0, -0.0*masses[me_idx], 0);
	externalForces[me_idx] = make_float4(0, -9820*masses[me_idx], 0, 0); // for mm.
//	externalForces[me_idx] = make_float4(0, -9.82*masses[me_idx], 0);
}
